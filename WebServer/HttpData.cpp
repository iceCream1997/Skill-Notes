...
AnalysisState HttpData::analysisRequest()
{
    ...
    else if (method_ == METHOD_GET || method_ == METHOD_HEAD)
    {
        ...
        if (fileName_ == "favicon.ico")
        {
            ...
            header += "Server: WhiteNotWolf's Web Server\r\n";
        ...
        if (fileName_ == "index.html")
        {
                ...
                tmp << myfile.rdbuf();
                std::string replay = tmp.str();
  
                header += "Content-Type: text/html;charset=UTF-8\r\n";
                header += "Content-Length: " + to_string(replay.length()) + "\r\n";
                header += "Server: WhiteNotWolf's Web Server\r\n";

                header += "\r\n";
                outBuffer_ += header;
                outBuffer_ += replay;
                LOG << "Replay: " << outBuffer_;
                myfile.close();
                return ANALYSIS_SUCCESS;
...
void HttpData::handleError(int fd, int err_num, string short_msg)
{
    ...
    body_buff += "<hr><em> WhiteNotWolf's Web Server</em>\n</body></html>";

    header_buff += "HTTP/1.1 " + to_string(err_num) + short_msg + "\r\n";
    header_buff += "Content-Type: text/html\r\n";
    header_buff += "Connection: Close\r\n";
    header_buff += "Content-Length: " + to_string(body_buff.size()) + "\r\n";
    header_buff += "Server: WhiteNotWolf's Web Server\r\n";;
    header_buff += "\r\n";
    // 错误处理不考虑writen不完的情况
